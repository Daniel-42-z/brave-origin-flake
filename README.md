...
```bash
nix run github:yourusername/brave-origin-flake
```

## Maintenance

This flake uses an automated script to stay up-to-date with upstream.
The `.github/workflows/update.yml` runs daily to check the Brave APT repository, hash the new `.deb` file, update `versions.json`, and automatically merge a Pull Request if checks pass.
